...
// Track if tables have been created in this Lambda execution
let tablesCreated = false;

async function createTables() {
  if (tablesCreated) {
    console.log("Tables already created in this execution, skipping...");
    return;
  }

  const models = [
    { name: "Transaction", model: Transaction },
    { name: "UserCourseProgress", model: UserCourseProgress },
    { name: "Course", model: Course },
  ];

  for (const { name, model } of models) {
    try {
      const table = new dynamoose.Table(name, [model], {
        create: true,
        update: false,
        waitForActive: true,
      ...
      console.log(`Table created and initialized: ${name}`);
    } catch (error: any) {
      if (
          error.message?.includes("already been assigned") ||
          error.message?.includes("Setup flow is already running")
      ) {
        console.log(`Table ${name} already exists or being set up, continuing...`);
      } else {
...
async function deleteAllTables() {
  ...
  if (TableNames && TableNames.length > 0) {
    ...
    console.log("Waiting for all tables to be fully deleted...");
...
export default async function seed() {
  ...
  const seedDataPath = path.join(process.cwd(), "src/seed/data");
  const files = fs
